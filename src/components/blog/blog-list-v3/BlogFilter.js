...
const fmtDay = (date) => {
  // supports blog.date?.day or blog.createdAt
...
const fmtMonth = (date) => {
  // supports blog.date?.month or blog.createdAt
...
export default function BlogFilter() {
  ...
  // build categories dynamically from data, but keep your default list as fallback
  const dynamicCats = useMemo(() => {
    const set = new Set();
    blogs.forEach((b) => b?.category && set.add(String(b.category)));
    const arr = Array.from(set);
    return arr.length ? ["All", ...arr] : [
      "All",
      "Home Improvement",
      "Life & Style",
      "Finance",
  ...
  // reset to page 1 if filters or data change
  useEffect(() => {
    setPage(1);
              ...
              <div className="blog-style1">
                <div className="blog-img">
                  <Image
                    ...
                    src={blog.image || blog.headerImageUrl || "/images/blogs/placeholder.jpg"}
                    alt={blog.title || "blog"}
                  />
                </div>
