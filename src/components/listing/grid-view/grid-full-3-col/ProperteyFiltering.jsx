...
export default function ProperteyFiltering() {
  ...
  const hookParams = Object.keys(serverParams).length
    ? serverParams
    : undefined;
  ...
  useEffect(() => {
    ...
    if (propertyTypes.length > 0) {
      andFilters.push(
        refItems.filter((el) =>
          propertyTypes.includes(el.propertyType || el.category)
        )
      );
    }
    ...
    if (bathroms > 0) {
      andFilters.push(
        refItems.filter((el) => (el.bathrooms ?? el.bath ?? 0) >= bathroms)
      );
    }
    ...
    if (categories.length > 0) {
      andFilters.push(
        refItems.filter((el) =>
          categories.every((c) =>
            (el.amenities || el.features || []).includes(c)
          )
        )
      );
    }
    ...
    if (location !== "All Cities") {
      andFilters.push(
        refItems.filter((el) => (el.city || "").toString() === location)
      );
    }
    ...
    const nextFiltered =
      andFilters.length === 0
        ...
        : refItems.filter((item) =>
            andFilters.every((arr) => arr.includes(item))
          );
    ...
    setFilteredData((prev) => {
      ...
      const sameIds =
        ...
        prev.every(
          (p, i) =>
            (p._id ?? p.id ?? i) ===
            (nextFiltered[i]?._id ?? nextFiltered[i]?.id ?? i)
        );
